...
export async function generateIncrementalSummary(req, res) {
  ...
  try {
    ...
    try {
      console.log(
        "calling the jsoneMerger.js function to merge all converted jsons..."
      );
      mergedJson = await jsoneMerger(jsonConvertedParagraphs);
      ...
      return res.status(500).json({ error: "Failed to merge JSON" });
    }
    ...
    try {
      ...
      return res.status(500).json({ error: "Failed to convert JSON to text" });
    }

    // Send the final text to the frontend
    console.log("Sending final incremental summary to the client");
    return res.status(200).json({ summary: finalText });
  } catch (err) {
    console.log(
